...
export const evalNotAvailableMsg = localize(
	"eval.not.available",
	"not available",
);
...
export const noRestartFrame = localize(
	"restartFrame.cannot",
	"Can't restart frame",
);
...
export function attributePathNotExist(
	attribute: string,
	path: string,
 ...
 */
export function attributePathRelative(
	attribute: string,
	path: string,
): DebugProtocol.Message {
	return new ErrorWithMessage(
...
export function runtimeConnectionTimeout(
	timeoutMs: number,
	errMsg: string,
): DebugProtocol.Message {
	return new ErrorWithMessage({
		...
		format: localize(
			"VSND2010",
			"Cannot connect to runtime process, timeout after {0} ms - (reason: {1}).",
			"{_timeout}",
			"{_error}",
		),
...
export function failedToReadPortFromUserDataDir(
	dataDirPath: string,
	err: Error,
) {
	return new ErrorWithMessage({
		...
		format: localize(
			"failed.to.read.port",
			"Failed to read file {dataDirPath}, {error}",
		),
...
export function activePortFileContentsInvalid(
	dataDirPath: string,
	dataDirContents: string,
) {
	return new ErrorWithMessage({
		id: 2034,
		format: localize(
			"port.file.contents.invalid",
			'File at location: "{dataDirPath}" did not contain valid port data, contents were: {dataDirContents}',
		),
