...
import { ISourcePathDetails, MappedPosition, SourceMap } from "./sourceMap";
...
export class SourceMaps {
	...
	public constructor(
		...
		this._sourceMapFactory = new SourceMapFactory(
			pathMapping,
	 ...
	 */
	public async processNewSourceMap(
		pathToGenerated: string,
		originalUrlToGenerated: string | undefined,
		sourceMapURL: string,
		isVSClient = false,
	): Promise<void> {
		const sourceMap = await this._sourceMapFactory.getMapForGeneratedPath(
			pathToGenerated,
