...
import {
	MappedPosition as MappedPositionSM,
	SourceMapConsumer,
} from "source-map";
...
export class SourceMap {
			...
			this._allSourcePathDetails = this._sources
				.map((inferredPath, i) => {
					...
					return <ISourcePathDetails>{
						inferredPath,
						originalPath,
						startPosition: this.generatedPositionFor(
							inferredPath,
							0,
							0,
						),
	...
	public constructor(
		...
		sourceMapPathOverrides?: utils.IStringDictionary<string>,
		isVSClient = false,
	) {
		...
		// Absolute path
		const computedSourceRoot = sourceMapUtils.getComputedSourceRoot(
			sm.sourceRoot,
			this._generatedPath,
			pathMapping,
		);
		...
		this._sources = sm.sources.map((sourcePath) => {
			if (sourceMapPathOverrides) {
				const fullSourceEntry = sourceMapUtils.getFullSourceEntry(
					this._originalSourceRoot,
					sourcePath,
				);
			...
			if (!path.isAbsolute(sourcePath)) {
				...
				sourcePath = utils.properResolve(
					computedSourceRoot,
					sourcePath,
				);
			}
	...
	public authoredPositionFor(line: number, column: number): MappedPosition {
		...
		if (position.source) {
			...
			position.source =
				this._authoredPathCaseMap.get(position.source) ||
				position.source;
