 ...
 */
export function resolveRelativeToFile(
	absPath: string,
	relPath: string,
): string {
 ...
 */
export function getComputedSourceRoot(
	sourceRoot: string,
	...
	if (sourceRoot) {
		if (sourceRoot.startsWith("file:///")) {
			...
			absSourceRoot =
				chromeUtils.applyPathMappingsToTargetUrlPath(
					sourceRoot,
					pathMapping,
				) || sourceRoot;
		...
		logger.log(
			`SourceMap: resolved sourceRoot ${sourceRoot} -> ${absSourceRoot}`,
		);
	} else if (path.isAbsolute(generatedPath)) {
		...
		const urlPathname =
			url.parse(generatedPath).pathname || "/placeholder.js"; // could be debugadapter://123, no other info.
		const mappedPath = chromeUtils.applyPathMappingsToTargetUrlPath(
			urlPathname,
			pathMapping,
		);
 ...
 */
export function applySourceMapPathOverrides(
	sourcePath: string,
	...
	const sortedOverrideKeys = Object.keys(sourceMapPathOverrides).sort(
	...
	for (let leftPattern of sortedOverrideKeys) {
		...
		// Does it match?
		const escapedLeftPattern = utils.escapeRegexSpecialChars(
			leftPattern,
			"/*",
		);
