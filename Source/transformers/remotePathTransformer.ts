...
export class RemotePathTransformer extends UrlPathTransformer {
	...
	private async init(args: ICommonRequestArgs): Promise<void> {
		if (
			(args.localRoot && !args.remoteRoot) ||
			(args.remoteRoot && !args.localRoot)
		) {
			throw new Error(
				localize(
					"localRootAndRemoteRoot",
	...
	public async stackTraceResponse(
		...
		await Promise.all(
			response.stackFrames.map((stackFrame) =>
				this.fixSource(stackFrame.source),
			),
		);
	}
	...
	private shouldMapPaths(remotePath: string): boolean {
		...
		return (
			...
			(path.posix.isAbsolute(remotePath) ||
				...
				utils.isFileUrl(remotePath))
		);
	}

	public getClientPathFromTargetPath(remotePath: string): string {
		remotePath =
			super.getClientPathFromTargetPath(remotePath) || remotePath;
	...
	public getTargetPathFromClientPath(localPath: string): string {
		...
		remotePath = utils.fixDriveLetterAndSlashes(
			remotePath,
