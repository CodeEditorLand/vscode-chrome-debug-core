...
export class UrlPathTransformer extends BasePathTransformer {
	...
	public async scriptParsed(scriptUrl: string): Promise<string> {
		...
		if (!clientPath) {
			...
			if (!scriptUrl.startsWith(ChromeUtils.EVAL_NAME_PREFIX)) {
				logger.log(
					`Paths.scriptParsed: could not resolve ${scriptUrl} to a file with pathMapping/webRoot: ${JSON.stringify(this._pathMapping)}. It may be external or served directly from the server's memory (and that's OK).`,
				);
			}
		} else {
			logger.log(
	...
	public async stackTraceResponse(
		...
		await Promise.all(
			response.stackFrames.map((frame) => this.fixSource(frame.source)),
		);
	}
	...
	public async fixSource(source: DebugProtocol.Source): Promise<void> {
		if (source && source.path) {
			...
			const clientPath =
				this.getClientPathFromTargetPath(source.path) ||
