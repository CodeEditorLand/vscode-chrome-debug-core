...
import { ISourcePathDetails, MappedPosition } from "../sourceMaps/sourceMap";
...
export class BaseSourceMapTransformer {
	...
	public setBreakpoints(
		...
		requestSeq: number,
		ids?: number[],
	): { args: ISetBreakpointsArgs; ids: number[] } {
		if (!this._sourceMaps) {
			return { args, ids };
		}
		...
		if (args.source.path) {
			...
			if (mappedPath) {
				...
				this._sourceMaps
					...
					.forEach((sourcePath) => {
						...
						if (sourceBPs) {
							...
							args.breakpoints =
								args.breakpoints.concat(sourceBPs);
				...
				logger.log(
					`SourceMaps.setBP: SourceMaps are enabled but ${argsPath} is a runtime script`,
				);
			} else {
				// Source (or generated) file which is not loaded.
				logger.log(
					`SourceMaps.setBP: ${argsPath} can't be resolved to a loaded script. It may just not be loaded yet.`,
				);
			}
		} else {
			// No source.path
			...
			if (args.authoredPath) {
				...
				if (shouldFilter) {
					breakpoints = breakpoints.filter(
						(_, i) => i < args.originalBPs.length,
					);
				}

				breakpoints.forEach((bp, i) => {
					const mapped = this._sourceMaps.mapToAuthored(
						args.generatedPath,
						bp.line,
						bp.column,
					);
	 ...
	 */
	public async stackTraceResponse(
		response: IInternalStackTraceResponseBody,
	): Promise<void> {
		if (this._sourceMaps) {
	...
	public async fixSourceLocation(
		sourceLocation: ISourceLocation | IInternalStackFrame,
	): Promise<void> {
		if (!this._sourceMaps) {
			return;
		}
	...
	public async scriptParsed(
		pathToGenerated: string,
		originalUrlToGenerated: string | undefined,
		sourceMapURL: string,
	): Promise<string[]> {
		if (this._sourceMaps) {
			this._allRuntimeScriptPaths.add(
				this.fixPathCasing(pathToGenerated),
			);

			if (!sourceMapURL) return null;

			// Load the sourcemap for this new script and log its sources
			const processNewSourceMapP = this._sourceMaps.processNewSourceMap(
				pathToGenerated,
	...
	public scopesResponse(
		pathToGenerated: string,
		scopesResponse: IScopesResponseBody,
	): void {
		if (this._sourceMaps) {
			scopesResponse.scopes.forEach((scope) =>
				this.mapScopeLocations(pathToGenerated, scope),
			);
		}
	}

	private mapScopeLocations(
		pathToGenerated: string,
		...
		let mappedStart = this._sourceMaps.mapToAuthored(
			pathToGenerated,
			scope.line,
			scope.column,
		);
		...
		if (!mappedStart) {
			mappedStart = this._sourceMaps.mapToAuthored(
				pathToGenerated,
				scope.line + 1,
				scope.column,
			);
		...
		if (mappedStart) {
			...
			const mappedEnd = this._sourceMaps.mapToAuthored(
				pathToGenerated,
				scope.endLine,
				scope.endColumn,
			);
	...
	public async getGeneratedPathFromAuthoredPath(
		authoredPath: string,
	): Promise<string> {
		...
		return (
			this._sourceMaps.getGeneratedPathFromAuthoredPath(authoredPath) ||
	...
	public async allSourcePathDetails(
		pathToGenerated: string,
	): Promise<ISourcePathDetails[]> {
		if (!this._sourceMaps) return [];
