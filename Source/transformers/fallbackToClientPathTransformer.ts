...
export class FallbackToClientPathTransformer extends UrlPathTransformer {
		...
		return super.targetUrlToClientPath(scriptUrl).then((filePath) => {
			...
			return filePath || ChromeUtils.isEvalScript(scriptUrl)
				...
				: // In any other case we ask the client to map it as a fallback, and return filePath if there is any failures
					this.requestClientToMapURLToFilePath(scriptUrl).catch(
						(rejection) => {
							logger.log(
	...
	private async requestClientToMapURLToFilePath(
		url: string,
	): Promise<string> {
		return new Promise<string>((resolve, reject) => {
			this._session.sendRequest(
				"mapURLToFilePath",
				{ url: url },
				FallbackToClientPathTransformer.ASK_CLIENT_TO_MAP_URL_TO_FILE_PATH_TIMEOUT,
				(response) => {
					if (response.success) {
						logger.log(
						...
						reject(
							new Error(
								`The client responded that the url "${url}" couldn't be mapped to a file path due to: ${response.message}`,
							),
						);
					}
				},
