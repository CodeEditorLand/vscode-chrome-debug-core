...
export class EagerSourceMapTransformer extends BaseSourceMapTransformer {
	...
	private discoverSourceMapForGeneratedScript(
		...
		return this.findSourceMapUrlInFile(generatedScriptPath)
			...
			.catch((err) => {
				...
				logger.log(
					`SourceMaps: could not preload for generated script: ${generatedScriptPath}. Error: ${err.toString()}`,
				);
			});
	}
	 ...
	 */
	private findSourceMapUrlInFile(
