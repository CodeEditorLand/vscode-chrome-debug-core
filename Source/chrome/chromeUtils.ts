 ...
 */
export function applyPathMappingsToTargetUrlPath(
...
export function applyPathMappingsToTargetUrl(
	scriptUrl: string,
	pathMapping: IPathMapping,
): string {
 ...
 */
export async function targetUrlToClientPath(
	aUrl: string,
	pathMapping: IPathMapping,
): Promise<string> {
	if (!aUrl) {
 ...
 */
export function remoteObjectToValue(
...
export function remoteObjectToCallArgument(
	object: Crdp.Runtime.RemoteObject,
): Crdp.Runtime.CallArgument {
	return {
 ...
 */
export function errorMessageFromExceptionDetails(
	exceptionDetails: Crdp.Runtime.ExceptionDetails,
): string {
...
export function getEvaluateName(
...
export function selectBreakpointLocation(
	lineNumber: number,
	columnNumber: number,
	locations: Crdp.Debugger.BreakLocation[],
): Crdp.Debugger.BreakLocation {
...
export async function getLaunchedPort(userDataDir: string): Promise<number> {
	...
	try {
		...
		return Promise.reject(
			errors.failedToReadPortFromUserDataDir(userDataDir, err),
		);
	}
}
