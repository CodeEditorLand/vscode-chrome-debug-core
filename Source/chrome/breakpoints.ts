...
import { BreakpointSetResult, IPendingBreakpoint } from "./chromeDebugAdapter";
...
export class Breakpoints {
	...
	private _committedBreakpointsByUrl = new Map<
		string,
		ISetBreakpointResult[]
	>();
	...
	private getValueFromCommittedBreakpointsByUrl(
		url: string,
	): ISetBreakpointResult[] {
	...
	private setValueForCommittedBreakpointsByUrl(
		url: string,
	...
	public setBreakpoints(
		...
		return this.validateBreakpointsPath(args).then(
			() => {
				...
				if (targetScriptUrl) {
					...
					const setBreakpointsPFailOnError =
						this._setBreakpointsRequestQ
							.then(() =>
								this.clearAllBreakpoints(targetScriptUrl),
							)
							.then(() =>
								this.addBreakpoints(
									targetScriptUrl,
							...
							.then((responses) => ({
								breakpoints:
									this.targetBreakpointResponsesToBreakpointSetResults(
										targetScriptUrl,
										responses,
										internalBPs,
										ids,
					...
					const setBreakpointsPTimeout = utils.promiseTimeout(
						setBreakpointsPFailOnError,
						Breakpoints.SET_BREAKPOINTS_TIMEOUT,
						localize(
							"setBPTimedOut",
					...
					this._setBreakpointsRequestQ = setBreakpointsPTimeout.catch(
						(e) => {
							// Log the timeout, but any other error will be logged elsewhere
							if (
								e.message &&
								e.message.indexOf("timed out") >= 0
							) {
					...
					return setBreakpointsPFailOnError.then(
						(setBpResultBody) => {
							const body = {
								breakpoints: setBpResultBody.breakpoints.map(
	...
	protected validateBreakpointsPath(
		...
		return this.adapter.sourceMapTransformer
			.getGeneratedPathFromAuthoredPath(args.source.path)
			.then<void>((mappedPath) => {
				if (!mappedPath) {
					return utils.errP(
						localize(
	...
	protected async addBreakpoints(
		...
		if (ChromeUtils.isEvalScript(url)) {
			...
			if (!this.adapter.breakOnLoadActive || script) {
				...
				// Else if script hasn't been parsed and break on load is active, we need to do extra processing
				if (this.adapter.breakOnLoadActive) {
					return await this.adapter.breakOnLoadHelper.handleAddBreakpoints(
						url,
						breakpoints,
					);
				}
	 ...
	 */
	async addOneBreakpointByUrl(
		scriptId: Crdp.Runtime.ScriptId | undefined,
		urlRegex: string,
		lineNumber: number,
		columnNumber: number,
		condition: string,
		...
		try {
			result = await this.chrome.Debugger.setBreakpointByUrl({
				urlRegex,
				lineNumber: bpLocation.lineNumber,
				columnNumber: bpLocation.columnNumber,
				condition,
			});
		} catch (e) {
			if (
				e.message === "Breakpoint at specified location already exists."
			) {
				return {
					actualLocation: {
						lineNumber: bpLocation.lineNumber,
	 ...
	 * @param ids IDs passed in for previously unverified breakpoints
	 */
	public async getBreakpointsLocations(
		...
		if (args.source.path) {
			...
			const endArgs = this.adapter.sourceMapTransformer.setBreakpoints(
				{
					breakpoints: [
						{ line: args.endLine, column: args.endColumn },
					],
					source: args.source,
				},
				requestSeq,
				...
				const end =
					typeof args.endLine === "number"
						? {
								...
								lineNumber: args.endLine,
								columnNumber: args.endColumn || 0,
							}
						: {
								scriptId: script.scriptId,
	 ...
	 */
	private targetBreakpointResponsesToBreakpointSetResults(
		url: string,
		responses: ISetBreakpointResult[],
		requestBps: InternalSourceBreakpoint[],
		ids?: number[],
	): BreakpointSetResult[] {
		// Don't cache errored responses
		const committedBps = responses.filter(
			(response) => response && response.breakpointId,
		);
		...
		return responses.map((response, i) => {
			...
			if (ids && ids[i]) {
				...
				bpId =
					this._breakpointIdHandles.lookup(responseBpId) ||
	...
	private addHitConditionBreakpoint(
		requestBp: InternalSourceBreakpoint,
		response: ISetBreakpointResult,
	): boolean {
		...
		if (result && result.length >= 3) {
			...
			const shouldPause: (numHits: number) => boolean = <any>(
				new Function("numHits", expr)
			);
			/* tslint:enable:no-function-constructor-with-string-args */
			this._hitConditionBreakpointsById.set(response.breakpointId, {
				numHits: 0,
				shouldPause,
			});
	...
	public onBreakpointResolved(
		params: Crdp.Debugger.BreakpointResolvedEvent,
		scripts: ScriptContainer,
	): void {
		...
		if (
			this.adapter.breakOnLoadActive &&
			this.adapter.breakOnLoadHelper.stopOnEntryBreakpointIdToRequestedFileName.has(
				params.breakpointId,
			)
		) {
			return;
		...
		const committedBps =
			this.getValueFromCommittedBreakpointsByUrl(script.url) || [];

		if (
			!committedBps.find(
				(committedBp) =>
					committedBp.breakpointId === params.breakpointId,
			)
		) {
			committedBps.push({
		...
		const scriptPath = utils.canonicalizeUrl(
			this.adapter.pathTransformer.breakpointResolved(bp, script.url),
		);
	...
	private unverifiedBpResponse(
		...
		const breakpoints = args.breakpoints.map((bp) => {
			return <DebugProtocol.Breakpoint>{
				...
				column: bp.column,
				message,
				id: this._breakpointIdHandles.create(
					this.generateNextUnboundBreakpointId(),
				),
			};
		});
	...
	private unverifiedBpResponseForBreakpoints(
		args: ISetBreakpointsArgs,
		requestSeq: number,
		targetScriptUrl: string,
		...
		if (args.source.path) {
			...
			this._pendingBreakpointsByUrl.set(
				utils.canonicalizeUrl(args.source.path),
				{
					...
					ids,
					requestSeq,
					setWithPath: this.adapter.breakOnLoadActive
						? ""
						: targetScriptUrl,
				},
	...
	public async handleScriptParsed(
		...
		if (sources) {
			const filteredSources = sources.filter(
				(source) => source !== mappedUrl,
			); // Tools like babel-register will produce sources with the same path as the generated script
		...
		if (
			utils.canonicalizeUrl(script.url) === mappedUrl &&
			this._pendingBreakpointsByUrl.has(mappedUrl) &&
			utils.canonicalizeUrl(
				this._pendingBreakpointsByUrl.get(mappedUrl).setWithPath,
			) === utils.canonicalizeUrl(mappedUrl)
		) {
	...
	public async resolvePendingBPs(source: string, scripts: ScriptContainer) {
		...
		if (
			...
			(!pendingBP.setWithPath ||
				utils.canonicalizeUrl(pendingBP.setWithPath) === source)
		) {
			logger.log(
			...
			if (
				...
				logger.log(
					`OnScriptParsed.resolvePendingBPs: The following pending breakpoints won't be resolved: ${JSON.stringify(pendingBP)} pendingBreakpointsByUrl = ${JSON.stringify([...this._pendingBreakpointsByUrl])} source = ${source}`,
				);
			}
		}
	}

	public resolvePendingBreakpoint(
	...
	public handleHitCountBreakpoints(
		expectingStopReason: ReasonType,
		hitBreakpoints,
	) {
		...
		for (let hitBp of hitBreakpoints) {
			if (this._hitConditionBreakpointsById.has(hitBp)) {
				...
				if (
					!expectingStopReason &&
					!hitConditionBp.shouldPause(hitConditionBp.numHits)
				) {
