...
import { ChromeDebugAdapter, CrdpScript } from "./chromeDebugAdapter";
...
export class ScriptContainer {
	 ...
	 */
	public get loadedScripts() {
		return this._scriptsById.values();
	}
	...
	public getOneScriptString(
		...
		return sourceMapTransformer
			.allSourcePathDetails(clientPath || runtimeScriptPath)
			.then((sourcePathDetails) => {
				let mappedSourcesStr = sourcePathDetails
					.map(
						(details) =>
	 ...
	 */
	public getSourceReferenceForScriptId(
		scriptId: Crdp.Runtime.ScriptId,
	): number {
		return (
			this._sourceHandles.lookupF(
				(container) => container.scriptId === scriptId,
	 ...
	 */
	public getSourceReferenceForScriptPath(
		mappedPath: string,
		contents: string,
	): number {
		return (
			this._sourceHandles.lookupF(
				(container) => container.mappedPath === mappedPath,
	 ...
	 */
	public async scriptToSource(
		script: Crdp.Debugger.ScriptParsedEvent,
		origin: string,
		remoteAuthority?: string,
	): Promise<DebugProtocol.Source> {
