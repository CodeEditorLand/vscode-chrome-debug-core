...
export abstract class BaseVariableContainer implements IVariableContainer {
	...
	public expand(
		...
		return variablesManager.getVariablesForObjectId(
			this.objectId,
			this.evaluateName,
			filter,
			start,
...
export class LoggedObjects extends BaseVariableContainer {
	...
	public expand(
		...
		return Promise.all(
			this.args.map((arg, i) =>
				variablesManager.remoteObjectToVariable(
					"" + i,
					arg,
					undefined,
					/*stringify=*/ false,
...
export class ScopeContainer extends BaseVariableContainer {
	...
	public setValue(
		...
		return variablesManager.setVariableValue(
			this._frameId,
			this._origScopeIndex,
			name,
			value,
		);
	}

	private insertRemoteObject(
		variablesManager: VariablesManager,
		variables: DebugProtocol.Variable[],
		name: string,
		obj: Crdp.Runtime.RemoteObject,
	): Promise<DebugProtocol.Variable[]> {
...
export class ExceptionValueContainer extends ExceptionContainer {
	...
	public expand(
		...
		const excValuePropDescriptor: Crdp.Runtime.PropertyDescriptor = <any>{
			name: "Exception",
...
function getArrayPreview(
	...
	if (object.preview) {
		...
		const propsWithIdx = indexedProps.slice(0, numProps).map((prop, i) => {
			return {
...
function getObjectPreview(
	...
	if (object.preview) {
		...
		if (
			object.preview.overflow ||
			object.preview.properties.length > numProps
		) {
			propsPreview += ", …";
		}
...
function trimProperty(value: string): string {
	return value !== undefined &&
		value !== null &&
		value.length > PREVIEW_PROP_LENGTH
		? value.substr(0, PREVIEW_PROP_LENGTH) + ELLIPSIS
		: value;
}

export function getRemoteObjectPreview(
	object: Crdp.Runtime.RemoteObject,
	stringify = true,
	context?: string,
): string {
...
export function getRemoteObjectPreview_object(
	object: Crdp.Runtime.RemoteObject,
	context?: string,
): string {
...
export function getRemoteObjectPreview_primitive(
	object: Crdp.Runtime.RemoteObject,
	stringify?: boolean,
): string {
...
export function getRemoteObjectPreview_function(
	object: Crdp.Runtime.RemoteObject,
	context?: string,
): string {
...
export class VariableHandles {
	...
	public get(handle: number): IVariableContainer {
		return (
			this._variableHandles.get(handle) ||
			this._consoleVariableHandles.get(handle)
		);
	}
...
export function getCollectionNumPropsByPreview(
	object: Crdp.Runtime.RemoteObject,
): IPropCount {
...
export function getArrayNumPropsByPreview(
	object: Crdp.Runtime.RemoteObject,
): IPropCount {
	let indexedVariables = 0;
	...
	if (indexedProps.length) {
		...
		indexedVariables =
			parseInt(indexedProps[indexedProps.length - 1].name, 10) + 1;
	}

	const namedVariables =
		object.preview.properties.length - indexedProps.length + 2; // 2 for __proto__ and length
	return { indexedVariables, namedVariables };
}

export function createPrimitiveVariableWithValue(
	name: string,
	value: string,
...
export function createFunctionVariable(
	name: string,
	object: Crdp.Runtime.RemoteObject,
	...
	return <DebugProtocol.Variable>{
		...
		variablesReference: handles.create(
			new PropertyContainer(object.objectId, evaluateName),
			context,
		),
		evaluateName,
	};
