...
export class BreakOnLoadHelper {
	...
	private _stopOnEntryBreakpointIdToRequestedFileName = new Map<
		string,
		UrlRegexAndFileSet
	>();
	...
	public constructor(
		chromeDebugAdapter: ChromeDebugAdapter,
	...
	validateStrategy(breakOnLoadStrategy: BreakOnLoadStrategy): void {
		if (
			...
			throw new Error(
				"Invalid breakOnLoadStrategy: " + breakOnLoadStrategy,
			);
		}
	}

	public get stopOnEntryRequestedFileNameToBreakpointId(): Map<
		string,
		string
	> {
	...
	public get stopOnEntryBreakpointIdToRequestedFileName(): Map<
		string,
		UrlRegexAndFileSet
	> {
	...
	private getScriptUrlFromId(scriptId: string): string {
		return utils.canonicalizeUrl(
			this._chromeDebugAdapter.scriptsById.get(scriptId).url,
		);
	}
	...
	private isInstrumentationPause(
		notification: Crdp.Debugger.PausedEvent,
	): boolean {
		return (
	...
	private async shouldContinueOnStopOnEntryBreakpoint(
		pausedLocation: Crdp.Debugger.Location,
	): Promise<boolean> {
		...
		await this._chromeDebugAdapter.getBreakpointsResolvedDefer(
			pausedLocation.scriptId,
		).promise;

		const pausedScriptUrl = this.getScriptUrlFromId(
			pausedLocation.scriptId,
		);
		...
		const anyBreakpointsAtPausedLocation =
			committedBps.filter(
				(bp) =>
					bp.actualLocation &&
					bp.actualLocation.lineNumber ===
						pausedLocation.lineNumber &&
					bp.actualLocation.columnNumber ===
			...
			if (!regexAndFileNames) {
				...
				if (regexAndFileNames.fileSet.has(normalizedMappedUrl)) {
					...
					if (regexAndFileNames.fileSet.size === 0) {
						...
						logger.log(
							`Stop on entry breakpoint hit but still has remaining files. Keeping: ${bp} that was hit for: ${normalizedMappedUrl} because it's still needed for: ${Array.from(regexAndFileNames.fileSet.entries()).join(", ")}`,
						);
					}
				}
			}
	 ...
	 * Only used when using regex approach for break on load
	 */
	private async addStopOnEntryBreakpoint(
		url: string,
		...
		if (!this._stopOnEntryRequestedFileNameToBreakpointId.has(url)) {
			...
			this._stopOnEntryRequestedFileNameToBreakpointId.set(
				normalizedUrl,
				breakpointId,
			);

			let regexAndFileNames =
				this._stopOnEntryBreakpointIdToRequestedFileName.get(
					breakpointId,
				);
	 ...
	 */
	public async handleAddBreakpoints(
		url: string,
	...
	private async setStopOnEntryBreakpoint(
		...
		let result =
			await this._chromeDebugAdapter.chrome.Debugger.setBreakpointByUrl({
				urlRegex,
				lineNumber: 0,
				columnNumber: 0,
			});
	...
	private async removeBreakpointById(breakpointId: string): Promise<void> {
		return await this._chromeDebugAdapter.chrome.Debugger.removeBreakpoint({
	...
	public shouldResolvePendingBPs(mappedUrl: string): boolean {
		if (
			this._breakOnLoadStrategy === "regex" &&
