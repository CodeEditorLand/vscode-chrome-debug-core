...
import {
	IObservableEvents,
	IStepStartedEventsEmitter,
	StepProgressEventsEmitter,
} from "../executionTimingsReporter";
import * as telemetry from "../telemetry";
import * as utils from "../utils";
import {
	ITarget,
	ITargetDiscoveryStrategy,
	ITargetFilter,
} from "./chromeConnection";
...
export class TargetVersions {
	constructor(
		public readonly protocol: Version,
		public readonly browser: Version,
	) {}
}

export class ChromeTargetDiscovery
	implements
		ITargetDiscoveryStrategy,
		IObservableEvents<IStepStartedEventsEmitter>
{
	...
	async getAllTargets(
         ...
         */
		this.telemetry.reportEvent("targetCount", {
	...
	private async _getVersionData(
		...
		const jsonResponse = await utils
	...
	private async _getTargets(
		...
		const jsonResponse = await checkDiscoveryEndpoint(
			`http://${address}:${port}/json/list`,
		)
			.catch(() =>
				checkDiscoveryEndpoint(`http://${address}:${port}/json`),
			)
			.catch((e) =>
				utils.errP(
					localize(
						"attach.cannotConnect",
						"Cannot connect to the target: {0}",
						e.message,
					),
				),
			);
		...
		try {
			...
			try {
				...
				return utils.errP(
					localize(
						"attach.invalidResponse",
						"Response from the target seems invalid. Error: {0}. Response: {1}",
						e.message,
						jsonResponse,
					),
				);
		...
		if (Array.isArray(responseArray)) {
			...
			return utils.errP(
				localize(
					"attach.invalidResponseArray",
					"Response from the target seems invalid: {0}",
					jsonResponse,
				),
			);
		}
	...
	private _getMatchingTargets(
		...
		if (!filteredTargets.length) {
			throw new Error(
				localize(
					"attach.noMatchingTarget",
					"Can't find a valid target that matches: {0}. Available pages: {1}",
					targetUrl,
					JSON.stringify(targets.map((target) => target.url)),
				),
			);
		...
		if (!targetsWithWSURLs.length) {
			throw new Error(
				localize(
					"attach.devToolsAttached",
					"Can't attach to this target that may have Chrome DevTools attached: {0}",
					filteredTargets[0].url,
				),
			);
		}
