...
import {
	IAttachRequestArgs,
	ICommonRequestArgs,
	ICompletionsResponseBody,
	IDebugAdapter,
	IEvaluateResponseBody,
	IExceptionInfoResponseBody,
	IGetLoadedSourcesResponseBody,
	IInitializeRequestArgs,
	ILaunchRequestArgs,
	IRestartRequestArgs,
	IScopesResponseBody,
...
import {
	mapInternalSourceToRemoteClient,
	mapRemoteClientToInternalPath,
} from "../remoteMapper";
...
import { isIndexedPropName, ScopeContainer } from "./variables";
...
export abstract class ChromeDebugAdapter implements IDebugAdapter {
	...
	private _scriptIdToBreakpointsAreResolvedDefer = new Map<
		string,
		PromiseDefer<void>
	>();
	...
	private _loadedSourcesByScriptId = new Map<
	...
	public constructor(
		...
		this._transformers = {
			lineColTransformer: new (lineColTransformer || LineColTransformer)(
				this._session,
			),
			sourceMapTransformer: new (sourceMapTransformer ||
				EagerSourceMapTransformer)(this._scriptContainer),
	...
	private get originProvider() {
		return (url: string) => this.getReadonlyOrigin(url);
	}
	...
	public async launch(
		...
		if (args.pathMapping) {
			for (const urlToMap in args.pathMapping) {
				args.pathMapping[urlToMap] = utils.canonicalizeUrl(
					args.pathMapping[urlToMap],
				);
			}
		}
		...
		if (!args.__restart) {
            ...
            */
			telemetry.reportEvent("debugStarted", {
				request: "launch",
    ...
    */
	...
	protected commonArgs(args: ICommonRequestArgs): void {
		...
		if (args.breakOnLoadStrategy && args.breakOnLoadStrategy !== "off") {
			this._breakOnLoadHelper = new BreakOnLoadHelper(
				this,
				args.breakOnLoadStrategy,
			);
		}
	...
	protected async terminateSession(
		reason: string,
		_disconnectArgs?: DebugProtocol.DisconnectArguments,
		restart?: IRestartRequestArgs,
	): Promise<void> {
		...
		if (!this._hasTerminated) {
			...
			if (
				this._clientAttached ||
				(this._launchAttachArgs &&
					(<ILaunchRequestArgs>this._launchAttachArgs).noDebug)
			) {
	...
	protected hookConnectionEvents(): void {
		this.chrome.Debugger.on("paused", (params) => {
             ...
             */
			this.runAndMeasureProcessingTime(
				"target/notification/onPaused",
				async () => {
		...
		this.chrome.Debugger.on("breakpointResolved", (params) =>
			this._breakpoints.onBreakpointResolved(
				params,
		...
		this._chromeConnection.onClose(() =>
			this.terminateSession("websocket closed"),
		);
	}

	private async runAndMeasureProcessingTime(
	...
	protected runConnection(): Promise<void>[] {
		return [
			this.chrome.Console.enable().catch(() => {
			...
			this.chrome.Log.enable().catch(() => {}), // Not supported by all runtimes
	...
	protected async doAttach(
		...
		if (!this._chromeConnection.isAttached) {
			if (websocketUrl) {
				await this._chromeConnection.attachToWebsocketUrl(
					websocketUrl,
					extraCRDPChannelPort,
				);
			} else {
				await this._chromeConnection.attach(
					address,
					port,
					targetUrl,
					timeout,
            ...
            */
			...
			this._scriptSkipper.init(
				this._launchAttachArgs.skipFiles,
				this._launchAttachArgs.skipFileRegExps,
			);

			await this.initSupportedDomains();

			const maxDepth = this._launchAttachArgs.showAsyncStacks
				? ChromeDebugAdapter.ASYNC_CALL_STACK_DEPTH
				: 0;
			...
			if (this._breakOnLoadHelper) {
				this._breakOnLoadHelper.setBrowserVersion(
					(await this._chromeConnection.version).browser,
				);
			}
	...
	private async initSupportedDomains(): Promise<void> {
		try {
			...
			domainResponse.domains.forEach((domain) =>
				this._domains.set(<any>domain.name, domain),
			);
		} catch (e) {
	...
	protected onExecutionContextsCleared(): Promise<void> {
		...
		return this.doAfterProcessingSourceEvents(async () => {
			// This will not execute until all the on-flight 'new' source events have been processed
	...
	protected async onPaused(
		...
		if (this.breakOnLoadActive) {
			let shouldContinue =
				await this._breakOnLoadHelper.handleOnPaused(notification);

			if (shouldContinue) {
				this.chrome.Debugger.resume().catch((e) => {
					logger.error(
						"Failed to resume due to exception: " + e.message,
					);
				});
		...
		if (notification.reason === "exception") {
			...
			if (
				this._promiseRejectExceptionFilterEnabled &&
				!this._pauseOnPromiseRejections
			) {
			...
			notification.hitBreakpoints &&
			notification.hitBreakpoints.length
		) {
			reason = "breakpoint";

			const result = this._breakpoints.handleHitCountBreakpoints(
				expectingStopReason,
				notification.hitBreakpoints,
			);
		...
		if (shouldSmartStep) {
			...
			const sendStoppedEvent = () => {
				return this._session.sendEvent(
					new StoppedEvent2(
						reason,
						/*threadId=*/ ChromeDebugAdapter.THREAD_ID,
						this._exception,
					),
				);
			};
    ...
    */
	public async exceptionInfo(
		...
		if (this._exception) {
			...
			const message = isError
				? utils.firstLine(this._exception.description)
				: this._exception.description || this._exception.value;

			const formattedMessage = message && message.replace(/\*/g, "\\*");

			const response: IExceptionInfoResponseBody = {
				exceptionId:
					this._exception.className ||
				...
				details: {
					stackTrace:
						...
						(await this._stackFrames.mapFormattedException(
							this._exception.description,
							this._transformers,
						)),
					message,
	...
	private async detectColumnBreakpointSupport(
		...
		this.lineColTransformer.columnBreakpointsEnabled =
			this._columnBreakpointsEnabled;
	}

	public getBreakpointsResolvedDefer(scriptId: string): PromiseDefer<void> {
		const existingValue =
			this._scriptIdToBreakpointsAreResolvedDefer.get(scriptId);
	...
	protected async onScriptParsed(
		...
		try {
			this.doAfterProcessingSourceEvents(async () => {
				// This will block future 'removed' source events, until this processing has been completed
			...
			if (script.url) {
				...
				script.url =
					ChromeDebugAdapter.EVAL_NAME_PREFIX + script.scriptId;
			}

			this._scriptContainer.add(script);

			const mappedUrl = await this.pathTransformer.scriptParsed(
				script.url,
			);

			const sourceMapsP = this.sourceMapTransformer
				.scriptParsed(mappedUrl, script.url, script.sourceMapURL)
				.then(async (sources) => {
					if (this._hasTerminated) {
						return undefined;
					}

					await this._breakpoints.handleScriptParsed(
						script,
						this._scriptContainer,
						mappedUrl,
						sources,
			...
			if (this._initialSourceMapsP) {
				this._initialSourceMapsP = <Promise<any>>(
	...
	protected async sendLoadedSourceEvent(
		script: Crdp.Debugger.ScriptParsedEvent,
		loadedSourceEventReason: LoadedSourceEventReason = "new",
	): Promise<void> {
		const origin = this.getReadonlyOrigin(script.url);

		const source = await this._scriptContainer.scriptToSource(
			script,
			origin,
		);
			...
			case "removed":
				if (!this._loadedSourcesByScriptId.delete(script.scriptId)) {
					telemetry.reportEvent("LoadedSourceEventError", {
						issue: "Tried to remove non-existent script",
		...
		const scriptEvent = new LoadedSourceEvent(
    ...
    */
	public async toggleSkipFileStatus(
		...
		if (!(await this.isInCurrentStack(args))) {
			...
			this._scriptSkipper.toggleSkipFileStatus(
				args,
				this._scriptContainer,
				this._transformers,
			);
	...
	private async isInCurrentStack(
		args: IToggleSkipFileStatusArgs,
	): Promise<boolean> {
		const currentStack = await this.stackTrace({ threadId: undefined });

		if (args.path) {
			return currentStack.stackFrames.some(
				(frame) => frame.source && frame.source.path === args.path,
			);
		} else {
			return currentStack.stackFrames.some(
				(frame) =>
					frame.source &&
					frame.source.sourceReference === args.sourceReference,
			);
		}
	}
    ...
    */
	public async loadedSources(): Promise<IGetLoadedSourcesResponseBody> {
		const sources = await Promise.all(
			Array.from(this._scriptContainer.loadedScripts).map((script) =>
				this._scriptContainer.scriptToSource(
					script,
					this.getReadonlyOrigin(script.url),
				),
			),
		);
	...
	protected onConsoleAPICalled(
		...
		const result = formatConsoleArguments(
			event.type,
			event.args,
			event.stackTrace,
		);
	...
	private async logObjects(
		objs: Crdp.Runtime.RemoteObject[],
		isError = false,
		...
		this._currentLogMessage = this._currentLogMessage
			.then(async () => {
				...
				if (objs.length === 1 && objs[0].type === "string") {
					...
					if (isError) {
						msg = await this._stackFrames.mapFormattedException(
							msg,
							this._transformers,
						);
					}
				...
				if (stackTrace && stackTrace.callFrames.length) {
					const stackFrame = await this._stackFrames.mapCallFrame(
	...
	protected async onExceptionThrown(
		params: Crdp.Runtime.ExceptionThrownEvent,
	): Promise<void> {
		if (this._launchAttachArgs._suppressConsoleOutput) {
			return;
		}

		return (this._currentLogMessage = this._currentLogMessage
			.then(async () => {
				const formattedException = formatExceptionDetails(
			...
			const onConsoleAPICalledParams: Crdp.Runtime.ConsoleAPICalledEvent =
				{
					...
					args: params.message.parameters || [
						{ type: "string", value: params.message.text },
					],
    ...
    */
	public disconnect(args: DebugProtocol.DisconnectArguments): void {
		telemetry.reportEvent("FullSessionStatistics/SourceMaps/Overrides", {
    ...
    */
	public setBreakpoints(
		args: ISetBreakpointsArgs,
		_: ITelemetryPropertyCollector,
		requestSeq: number,
		ids?: number[],
	): Promise<ISetBreakpointsResponseBody> {
		if (args.source.path) {
    ...
    */
	public setExceptionBreakpoints(
		...
		return this.chrome.Debugger.setPauseOnExceptions({ state }).then(
			() => {},
		);
	}
    ...
    */
	...
	public continue(internal = false): Promise<void> {
        ...
        */
		...
		return (this._currentStep = this.chrome.Debugger.resume().then(
			() => {
				/* make void */
			},
			() => {
    ...
    */
	public stepBack(): Promise<void> {
		return (<TimeTravelRuntime>this.chrome).TimeTravel.stepBack().then(
			() => {
				/* make void */
			},
			() => {
    ...
    */
	public reverseContinue(): Promise<void> {
		return (<TimeTravelRuntime>this.chrome).TimeTravel.reverse().then(
			() => {
				/* make void */
			},
			() => {
    ...
    */
	public async stackTrace(
		...
		stackTraceResponse.stackFrames = stackTraceResponse.stackFrames.map(
			(frame) => {
				return {
					...frame,
					source: mapInternalSourceToRemoteClient(
						frame.source,
						this._launchAttachArgs.remoteAuthority,
					),
				};
	...
	public async variables(
		...
		return {
			variables: variables.filter(
				(v) =>
					ChromeDebugAdapter.FILTERED_VARIABLE_NAMES.indexOf(
						v.name,
					) === -1,
			),
		};
	...
	public source(
		...
		if (args.sourceReference) {
			...
			const script = this.getScriptByUrl(
				utils.isURL(realPath) ? encodeURI(realPath) : realPath,
			);

			if (!script) {
				return Promise.reject(
					errors.sourceRequestCouldNotRetrieveContent(),
				);
			}
		...
		return this.chrome.Debugger.getScriptSource({ scriptId }).then(
			(response) => {
				return {
    ...
    */
	public async evaluate(
		...
		const evalResponse = await this.waitThenDoEvaluate(
			args.expression,
			args.frameId,
		...
		const variable = await this._variablesManager.remoteObjectToVariable(
			args.expression,
			evalResponse.result,
			/*parentEvaluateName=*/ undefined,
			/*stringify=*/ undefined,
			<VariableContext>args.context,
		);
		...
		if (evalResponse.exceptionDetails) {
			...
			if (
				resultValue &&
				(resultValue.startsWith("ReferenceError: ") ||
					resultValue.startsWith("TypeError: ")) &&
				args.context !== "repl"
			) {
	 ...
	 */
	private handleScriptsCommand(
		args: DebugProtocol.EvaluateArguments,
	): Promise<IEvaluateResponseBody> {
		let outputStringP: Promise<string>;

		const scriptsRest = utils
			.lstrip(args.expression, ChromeDebugAdapter.SCRIPTS_COMMAND)
			.trim();
		...
		if (scriptsRest) {
			...
			if (requestedScript) {
				outputStringP = this.chrome.Debugger.getScriptSource({
					scriptId: requestedScript.scriptId,
				}).then((result) => {
					const maxLength = 1e5;
				...
				outputStringP = Promise.resolve(
					`No runtime script with url: ${scriptsRest}\n`,
				);
			}
		} else {
			outputStringP = this._scriptContainer.getAllScriptsString(
				this.pathTransformer,
				this.sourceMapTransformer,
			);
		}
	...
	private async _shouldSmartStepCallFrame(
		frame: Crdp.Debugger.CallFrame,
	): Promise<boolean> {
		const stackFrame = this._stackFrames.callFrameToStackFrame(
			frame,
			this._scriptContainer,
			this.originProvider,
		);
	...
	private async waitThenDoEvaluate(
		expression: string,
		frameId?: number,
		extraArgs?: Partial<Crdp.Runtime.EvaluateRequest>,
	): Promise<
		| Crdp.Debugger.EvaluateOnCallFrameResponse
		| Crdp.Runtime.EvaluateResponse
	> {
		...
		this._waitAfterStep = waitThenEval.then(
			() => {},
			() => {},
		); // to Promise<void> and handle failed evals
		return waitThenEval;
	}
    ...
    */
	public async restartFrame(
		...
		try {
			await this.chrome.Debugger.restartFrame({
    ...
    */
	public async completions(
	...
	): Promise<ICompletionsResponseBody> {
		...
		if (typeof args.frameId === "number" && !expression) {
			...
			const scopeExpandPs = callFrame.scopeChain.map((scope) =>
				new ScopeContainer(
			...
			return Promise.all(scopeExpandPs).then(
				(variableArrs: DebugProtocol.Variable[][]) => {
					const targets = this.getFlatAndUniqueCompletionItems(
						variableArrs.map((variableArr) =>
							variableArr.map((variable) => variable.name),
						),
					);
			...
			logger.verbose(
				`Completions: Returning for expression '${expression}'`,
			);
			...
			const response = await this.waitThenDoEvaluate(
				getCompletionsFn,
				args.frameId,
				{ returnByValue: true },
			);
			...
			if (response.exceptionDetails) {
				...
				return {
					targets: this.getFlatAndUniqueCompletionItems(
	...
	private getFlatAndUniqueCompletionItems(
		arrays: string[][],
	): DebugProtocol.CompletionItem[] {
	...
	public breakpointLocations(
		...
		_telemetryPropertyCollector?: ITelemetryPropertyCollector,
		requestSeq?: number,
	): Promise<DebugProtocol.BreakpointLocationsResponse["body"]> {
		return this._breakpoints.getBreakpointsLocations(
			args,
			this._scriptContainer,
			requestSeq,
		);
