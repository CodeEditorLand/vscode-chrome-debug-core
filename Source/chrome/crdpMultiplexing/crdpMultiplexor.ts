...
export class CRDPMultiplexor {
	...
	private onResponseMessage(message: { id: number }, data: string): void {
		...
		if (channel) {
			...
			throwCriticalError(
				`Didn't find channel for message with id: ${message.id} and data: <${data}>`,
			);
		}
	}

	private onDomainNotification(
	...
	public removeListenerOfNonMultiplexedEvent(
		event: string,
		cb: Function,
	): void {
...
export class CRDPChannel implements LikeSocket {
	...
	public callDomainMessageCallbacks(
		domain: string,
		messageData: string,
	): void {
	...
	constructor(
		public name: string,
		public id: number,
		private _multiplexor: CRDPMultiplexor,
	) {}
	...
	public on(event: string, cb: Function): void {
		if (event === "message") {
			if (this._messageCallbacks.length === 0) {
				setTimeout(
					() => this.discardUnsentPendingMessages(),
					CRDPChannel.timeToPreserveMessagesInMillis,
				);
			}
