...
export function formatExceptionDetails(
	e: Crdp.Runtime.ExceptionDetails,
): string {
	if (!e.exception) {
	...
	return (
		(e.exception.className &&
			e.exception.className.endsWith("Error") &&
...
export function formatConsoleArguments(
	type: Crdp.Runtime.ConsoleAPICalledEvent["type"],
	args: Crdp.Runtime.RemoteObject[],
	stackTrace?: Crdp.Runtime.StackTrace,
): { args: Crdp.Runtime.RemoteObject[]; isError: boolean } {
	switch (type) {
		case "log":
		case "debug":
		case "info":
	...
	for (let argIdx = 0; argIdx < args.length; argIdx++) {
		...
		if (typeof formatted === "string") {
			if (formatSpec) {
				...
				currentCollapsedStringArg = currentCollapsedStringArg.replace(
					"%" + formatSpec,
					formatted,
				);
			} else {
...
function formatArg(
	formatSpec: string,
	arg: Crdp.Runtime.RemoteObject,
): string | Crdp.Runtime.RemoteObject {
