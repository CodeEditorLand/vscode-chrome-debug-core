...
import {
	...
	IFinishedStartingUpEventsEmitter,
	IObservableEvents,
...
export class ChromeDebugSession
		...
		>
{
	...
	public constructor(
		...
		process.addListener("uncaughtException", (err: any) => {
			...
			logger.error(
				`******** Unhandled error in debug adapter: ${safeGetErrDetails(err)}`,
			);
		});

		process.addListener(
			"unhandledRejection",
			(err: Error | DebugProtocol.Message) => {
				reportErrorTelemetry(err, "unhandledRejection");

				// Node tests are watching for the ********, so fix the tests if it's changed
				logger.error(
					`******** Unhandled error in debug adapter - Unhandled promise rejection: ${safeGetErrDetails(err)}`,
				);
			},
		);
	}
	...
	private async reportTelemetry(
		eventName: string,
		action: (
			reportFailure: (failure: any) => void,
		...
		const sendTelemetry = () => {
			...
			if (isSequentialRequest) {
				...
				this.events.emitRequestCompleted(
					eventName,
					startTime,
	...
	private isEvaluateRequest(
		requestType: string,
		error: RequestHandleError,
	): boolean {
	...
	private failedRequest(
		...
		this.sendErrorResponse(
			...
			{
				_extensionName: this._extensionName,
	...
	private sendUnknownCommandResponse(
		response: DebugProtocol.Response,
		command: string,
	): void {
		this.sendErrorResponse(
			response,
			1014,
			`[${this._extensionName}] Unrecognized request: ${command}`,
			null,
			ErrorDestination.Telemetry,
		);
	}

	public reportTimingsWhileStartingUpIfNeeded(
		...
		if (!this.haveTimingsWhileStartingUpBeenReported) {
			...
			if (
				!requestedContentWasDetected &&
				typeof reasonForNotDetected !== "undefined"
			) {
				telemetryData.RequestedContentWasNotDetectedReason =
					reasonForNotDetected;
			}
		...
		this._debugAdapter.events.once(
			...
			(args) => {
				this.reportTimingsWhileStartingUpIfNeeded(
					args ? args.requestedContentWasDetected : true,
					args && args.reasonForNotDetected,
				);
			},
		);
		...
		setTimeout(
			() =>
				this.reportTimingsWhileStartingUpIfNeeded(
					...
					/*reasonForNotDetected*/ "timeout",
				),
			this._readyForUserTimeoutInMilliseconds,
		);
	}
...
function logVersionInfo(): void {
	...
	telemetry.addCustomGlobalProperty({
		"Versions.DebugAdapterCore": coreVersion,
	});
}
