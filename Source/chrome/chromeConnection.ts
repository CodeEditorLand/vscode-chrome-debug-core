...
import {
	ChromeTargetDiscovery,
	TargetVersions,
	Version,
} from "./chromeTargetDiscoveryStrategy";
...
export class ChromeConnection
	implements IObservableEvents<IStepStartedEventsEmitter>
{
	...
	constructor(
		targetDiscovery?: ITargetDiscoveryStrategy &
			IObservableEvents<IStepStartedEventsEmitter>,
		targetFilter?: ITargetFilter,
	) {
	...
	public attachToWebsocketUrl(
		...
		this._client.on("error", (e) =>
			logger.error("Error handling message from target: " + e.message),
		);
	}
	...
	public getAllTargets(
		...
		return this._targetDiscoveryStrategy.getAllTargets(
			address,
			port,
			targetFilter,
			targetUrl,
	...
	private _attach(
		...
		port: number,
		targetUrl?: string,
		timeout = ChromeConnection.ATTACH_TIMEOUT,
		extraCRDPChannelPort?: number,
	): Promise<void> {
		...
		return utils
			.retryAsync(
				() =>
					this._targetDiscoveryStrategy.getTarget(
						address,
						port,
						this._targetFilter,
						targetUrl,
			...
			.catch((err) =>
				Promise.reject(
					errors.runtimeConnectionTimeout(timeout, err.message),
				),
			)
			.then((target) => {
	...
	public get version(): Promise<TargetVersions> {
		return this._attachedTarget.version.then((version) => {
			return version
				? version
				: new TargetVersions(
