...
import { calculateElapsedTime, HighResTimer } from "./utils";
...
export class StepProgressEventsEmitter
	extends EventEmitter
	implements IStepStartedEventsEmitter, IFinishedStartingUpEventsEmitter
{
	constructor(
		private readonly _nestedEmitters: IStepStartedEventsEmitter[] = [] as IStepStartedEventsEmitter[],
	) {
		super();
	}

	public emitStepStarted(stepName: string): void {
		this.emit(stepStartedEventName, {
			stepName: stepName,
		} as IStepStartedEventArguments);
	}

	public emitMilestoneReached(milestoneName: string): void {
		this.emit(milestoneReachedEventName, {
			milestoneName: milestoneName,
		} as IMilestoneReachedEventArguments);
	}

	public emitStepCompleted(stepName: string): void {
		this.emit(stepCompletedEventName, {
			stepName: stepName,
		} as IStepCompletedEventArguments);
	}

	public emitRequestCompleted(
		requestName: string,
		requestStartTime: number,
		timeTakenByRequestInMilliseconds: number,
	): void {
		this.emit(requestCompletedEventName, {
			requestName: requestName,
			startTime: requestStartTime,
			timeTakenInMilliseconds: timeTakenByRequestInMilliseconds,
		} as IRequestCompletedEventArguments);
	}
...
class SubscriptionManager {
	...
	public removeAll(): void {
		for (const removeSubscriptionAction of this
			._removeSubscriptionActions) {
...
export class ExecutionTimingsReporter {
	...
	private addElementToArrayProperty<T>(
	...
	public subscribeTo(eventEmitter: EventEmitter): void {
		this._subscriptionManager.on(
			eventEmitter,
			stepStartedEventName,
			(args: IStepStartedEventArguments) => {
		...
		this._subscriptionManager.on(
			eventEmitter,
			milestoneReachedEventName,
			(args: IMilestoneReachedEventArguments) => {
		...
		this._subscriptionManager.on(
			...
			(args: IStepCompletedEventArguments) => {
				/* __GDPR__FRAGMENT__
               "StepNames" : {
                  "${wildcard}": [
                     {
             ...
             */
				this.recordTotalTimeUntilMilestone(
					`WaitingAfter.${args.stepName}`,
				);
			},
		);
		...
		this._subscriptionManager.on(
			...
			(args: IRequestCompletedEventArguments) => {
				this.recordRequestCompleted(
					args.requestName,
					args.startTime,
					args.timeTakenInMilliseconds,
				);
			},
		);
	}
