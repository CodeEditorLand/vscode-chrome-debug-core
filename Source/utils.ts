	...
	function tryUntilTimeout(): Promise<any> {
		return fn().catch((e) => {
			if (Date.now() - startTime < timeoutMs - intervalDelay) {
				return promiseTimeout(null, intervalDelay).then(
					tryUntilTimeout,
				);
			} else {
				return errP(e);
...
export function lstrip(s: string, lStr: string): string {
	return s.startsWith(lStr) ? s.substr(lStr.length) : s;
}
...
export function fillErrorDetails(
	properties: IExecutionResultTelemetryProperties,
	e: any,
): void {
