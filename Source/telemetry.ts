...
const prefixTelemetryKeys: ReadonlyArray<
	keyof IExecutionResultTelemetryProperties
> = ["exceptionStack", "exceptionMessage", "exceptionId"];
...
export class TelemetryReporter implements ITelemetryReporter {
	...
	public addCustomGlobalProperty(
		additionalGlobalTelemetryProperties: any,
	): void {
		Object.assign(
			this._globalTelemetryProperties,
			additionalGlobalTelemetryProperties,
		);
	}
}

// If you add an async global property, all events after that will include it
export class AsyncGlobalPropertiesTelemetryReporter
	implements ITelemetryReporter
{
	...
	public setupEventHandler(
		_sendEvent: (event: DebugProtocol.Event) => void,
	): void {
		this._telemetryReporter.setupEventHandler(_sendEvent);
	}

	public addCustomGlobalProperty(
		additionalGlobalPropertiesPromise: Promise<any> | any,
	): void {
		const reportedPropertyP = Promise.resolve(
			additionalGlobalPropertiesPromise,
		).then(
			(property) =>
	...
	private reportErrorWhileWaitingForProperty(rejection: any): void {
         ...
         */
		this._telemetryReporter.reportEvent(
			"error-while-adding-custom-global-property",
			properties,
		);
	}
...
export class BatchTelemetryReporter {
	...
	public constructor(
		private _telemetryReporter: ITelemetryReporter,
	 ...
	 */
	private static transfromBucketData(bucketForEventType: any[]): {
		[groupedPropertyValue: string]: string;
	} {
		const allPropertyNamesInTheBucket =
		...
		for (const event of bucketForEventType) {
			for (const propertyName of allPropertyNamesInTheBucket) {
				properties[`aggregated.${propertyName}`].push(
		...
		for (const propertyName of allPropertyNamesInTheBucket) {
			properties[`aggregated.${propertyName}`] = JSON.stringify(
				properties[`aggregated.${propertyName}`],
			);
		}
...
export const telemetry = new AsyncGlobalPropertiesTelemetryReporter(
