...
export function mapInternalSourceToRemoteClient(
	...
	if (
		source &&
		source.path &&
		isInternalRemotePath(source.path) &&
		remoteAuthority
	) {
		const remoteUri = URI.file(
			source.path.replace(
				new RegExp(remotePathComponent + "[\\/\\\\]"),
				"",
			),
		).with({
...
export function isInternalRemotePath(path: string): boolean {
	return (
		path.startsWith("/" + remotePathComponent) ||
		!!path.match(new RegExp("[a-zA-Z]:[\\/\\\\]" + remotePathComponent))
	);
}
