...
const libs = ["src"].map((libFolder) => libFolder + "/**/*.d.ts");
...
function doBuild(buildNls, failOnError) {
	...
	return merge([
		...
		tsResult.js
			...
			.pipe(
				buildNls
					? nls.bundleMetaDataFiles("vscode-chrome-debug-core", "out")
					: es.through(),
			)
			...
			.pipe(gulp.dest("out")),
		gulp.src(libs, { base: "." }).pipe(gulp.dest("lib")),
		gulp.src(testDataDir + "app*", { base: "." }).pipe(gulp.dest("out")),
	])
		.once("error", () => {
			gotError = true;
		})
...
gulp.task(
	...
	gulp.series("build", () => {
		return gulp
			.src(["out/nls.metadata.header.json", "out/nls.metadata.json"])
			.pipe(
				nls.createXlfFiles(
